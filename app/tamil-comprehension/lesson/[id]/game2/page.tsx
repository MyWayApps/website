...
export default function Game2Page() {
  ...
  const handleNext = async () => {
    ...
    const applicationId = `tamil-comprehension-lesson-${lessonId}-game2`
    await saveGameScore({
      userId: user.id,
      applicationId,
      score,
    ...
    const tier = await getActivityMasteryTier(user.id, applicationId, `tamil-comprehension:${lessonId}:game2`)
    setMasteryTier(tier)
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-200 to-pink-400 p-4 relative overflow-hidden">
      ...
      <div className="text-center mb-6">
        <h1 className="text-4xl font-bold text-purple-900 mb-2">
          {lesson.tamilTitle} - Question Set 2
        </h1>
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-base text-purple-700/70 italic mt-1">{romanize(getSentenceWithAnswer(), "tamil")}</p>
              </div>
              <Button
