...
import { barakhadiMatraSequence } from "@/lib/sanskrit-barakhadi-data"

export default function SanskritSequenceGame() {
  ...
  useEffect(() => {
    getApplicationByName("Sanskrit Barakhadi").then((application) => setApplicationId(application?.id ?? null))
  }, [])
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:sanskrit-barakhadi:sequence:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const generateQuestions = () => {
    const questions = []
    const totalMatras = barakhadiMatraSequence.length

    for (let i = 0; i < 5; i++) {
      const startIndex = Math.floor(Math.random() * (totalMatras - 5))
      const questionSequence = barakhadiMatraSequence.slice(startIndex, startIndex + 6)
      ...
      while (wrongOptions.length < 2) {
        ...
        const wrongMat = barakhadiMatraSequence[randomIndex]
  ...
  const handleBackToSequence = () => {
    window.location.href = "/sanskrit-barakhadi/sequence"
  }
