...
import { barakhadiMatraData, barakhadiOptionText } from "@/lib/sanskrit-barakhadi-data"

export default function SanskritMatchGame() {
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:sanskrit-barakhadi:match:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const shuffledMatras = [...barakhadiMatraData].sort(() => Math.random() - 0.5).slice(0, numPairs)

      // Create left side options (half-letter consonant + vowel formula)
      const leftOptions = shuffledMatras.map(mat => ({
        id: `left-${mat.matra}`,
        text: barakhadiOptionText(consonant, mat.matra, mat.name),
        matra: mat.matra,
        matraName: mat.name,
      ...
      const remainingMatras = barakhadiMatraData.filter(mat =>
  ...
  const handleBackToMatch = () => {
    window.location.href = "/sanskrit-barakhadi/match"
  }
