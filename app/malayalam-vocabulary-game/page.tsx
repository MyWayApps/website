...
export default function MalayalamVocabularyGame() {
  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Malayalam Vocabulary")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Malayalam Vocabulary scoring data:", error)
    }
  }
  ...
  useEffect(() => {
    ...
    const correct = pickUnseenRandom(`malayalam-vocabulary-game:${categoryId}`, 0, items.length - 1)
  ...
  if (!isReady || items.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-300 via-cyan-400 to-sky-500 p-4 flex flex-col items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-300 via-cyan-400 to-sky-500 p-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-[800px] flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white/30 px-4 py-2 rounded-full">
            <span className="text-cyan-900 font-bold">{categoryName.malayalam}</span>
          </div>
          <div className="flex items-center gap-2 bg-white/20 px-6 py-3 rounded-full backdrop-blur-sm">
            <Star className="h-6 w-6 text-yellow-500" />
            <span className="text-xl font-bold text-cyan-900">Score: {score}</span>
          </div>
        </div>
      </div>
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-cyan-900">
          Listen and find the Malayalam word!
        </h1>
      </div>
          ...
          {gameOver ? (
            ...
            <>
              <div className="mb-6 flex flex-col items-center">
                <div className="text-xl text-cyan-900 font-semibold mb-4">
                  Question {round} of {totalRounds}
                </div>

                {/* English Word Display */}
                <div className="bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl p-6 mb-4 text-center shadow-lg">
                  <div className="text-sm text-teal-600 mb-1">Listen to the English word:</div>
                  <div className="text-3xl font-bold text-teal-800 mb-3">
                    {items[correctIdx].english}
                  </div>
                  <Button
                    onClick={handleReplayAudio}
                    className="bg-teal-500 hover:bg-teal-600 text-white font-bold text-lg px-6 py-3"
              ...
              {/* Malayalam Word Choices */}
              <div className="text-center mb-4">
                <div className="text-lg text-cyan-700 font-semibold">Select the correct Malayalam word:</div>
              </div>
                  ...
                  <Card
                    ...
                    className={`p-6 flex items-center justify-center text-3xl md:text-4xl font-bold cursor-pointer transition-all duration-200 ${
                      showResult && choices[idx] === correctIdx && showResult === "correct"
                        ...
                        : "bg-white hover:bg-cyan-50 border-2 border-cyan-200 hover:border-cyan-400"
                    }`}
                    onClick={() => handleCardClick(idx)}
                  >
                    <div className="flex flex-col items-center">
                      <span className="text-cyan-800">{items[itemIdx].malayalam}</span>
                      <span className="text-base md:text-lg font-normal text-cyan-600/80 italic mt-1">
                        {romanize(items[itemIdx].malayalam, "malayalam")}
                      </span>
                    </div>
                  </Card>
                ))}
      ...
      {showConfetti && (
        <div className="fixed inset-0 pointer-events-none z-50">
          ...
          {[...Array(30)].map((_, i) => (
            ...
            >
              <div className="w-2 h-2 bg-sky-400 transform rotate-45"></div>
            </div>
          ))}
        </div>
      )}
