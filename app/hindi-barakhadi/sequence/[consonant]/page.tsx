...
import { barakhadiMatraSequence } from "@/lib/hindi-barakhadi-data"

export default function HindiSequenceGame() {
  ...
  useEffect(() => {
    getApplicationByName("Hindi Barakhadi").then((application) => setApplicationId(application?.id ?? null))
  }, [])
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:hindi-barakhadi:sequence:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const handleBackToSequence = () => {
    window.location.href = "/hindi-barakhadi/sequence"
  }
