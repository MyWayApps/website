...
import { barakhadiConsonants } from "@/lib/hindi-barakhadi-data"

export default function HindiSequencePickerMain() {
  const [selectedConsonant, setSelectedConsonant] = useState("")
  const router = useRouter()

  const handleBack = () => {
    window.location.href = "/hindi-barakhadi"
  }

  const handleStartGame = () => {
    if (selectedConsonant) {
      router.push(`/hindi-barakhadi/sequence/${selectedConsonant}`)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-200 to-indigo-400 p-4 flex flex-col items-center justify-center">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-xl text-indigo-700 font-semibold">
          Choose a consonant and arrange its Hindi letters in correct order
        </p>
      </div>
