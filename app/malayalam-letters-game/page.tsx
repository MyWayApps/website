...
import { getLettersByType, getLetterTypeLabel, LetterType, MalayalamLetter } from "@/lib/malayalam-letters-data"
...
export default function MalayalamLettersGame() {
  ...
  const [letters, setLetters] = useState<MalayalamLetter[]>([])
  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Malayalam Letters")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Malayalam Letters scoring data:", error)
    }
  }
    ...
    if (round > ROUND_LENGTH) {
      ...
      if (user && app) {
        saveGameScore({
          ...
          .then(() =>
            getActivityMasteryTier(user.id, app.id, `letters-game:malayalam`)
              .then(setMasteryTier)
              .catch((error) => console.error("Error fetching Malayalam Letters mastery tier:", error)),
          )
          .catch((error) => console.error("Error saving Malayalam Letters score:", error))
      }
      return
    }
    const correct = pickUnseenRandom(`malayalam-letters-game:${letterType}`, 0, letters.length - 1)
  ...
  const handleBackToHome = () => {
    window.location.href = "/malayalam-letters"
  }
  const handleRestart = () => {
    setScore(0)
    setRound(1)
        ...
        <CardContent className="p-8">
          {gameOver ? (
            <div className="flex flex-col items-center justify-center w-full">
              ...
              <div className="text-2xl text-cyan-900 mb-2">Your Score: {score} / {ROUND_LENGTH}</div>
              {masteryTier && (
                <div className="mb-4">
                  <MasteryBadge tier={masteryTier} showLabel />
                </div>
              )}
              <Button onClick={handleRestart} className="mt-4 bg-cyan-500 hover:bg-cyan-600 text-white font-bold px-6 py-3" variant="outline">
                Play Again
              </Button>
            </div>
          ) : (
            <>
              <div className="mb-6 flex flex-col items-center">
                <div className="text-xl text-cyan-900 font-semibold mb-3">
                  Question {round} of {ROUND_LENGTH}
                </div>
                <Button
                  onClick={handleReplayAudio}
                  disabled={isSpeaking}
                  className="bg-white/30 hover:bg-white/40 text-cyan-800 border-white font-bold text-lg px-6 py-3"
                  variant="outline"
                >
                  🔊 Hear the Letter
                </Button>
                    ...
                    className={`w-36 h-52 flex flex-col items-center justify-center gap-1 text-7xl font-bold cursor-pointer transition-all duration-200 ${
                      showResult && choices[idx] === correctIdx && showResult === "correct"
                        ...
                        : showResult && idx === choices.findIndex((i) => i === correctIdx) && showResult === "wrong"
                          ...
                          : "bg-white/80 hover:bg-cyan-100"
                    }`}
                    onClick={() => handleCardClick(idx)}
                  >
                    <span>{letters[letterIdx].letter}</span>
                    <span className="text-base font-normal opacity-70">
                      {romanize(letters[letterIdx].letter, "malayalam")}
                    </span>
                  </Card>
                ))}
              </div>
