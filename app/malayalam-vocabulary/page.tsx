...
import { vocabularyCategories } from "@/lib/malayalam-vocabulary-data"

export default function MalayalamVocabulary() {
        ...
        <Card className="w-full max-w-[900px] bg-white/90 backdrop-blur-sm shadow-2xl border-0 mb-6">
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Link href={`/malayalam-vocabulary-flashcards?category=${selectedCategory}`}>
              ...
              {/* Vocabulary Game */}
              <Link href={`/malayalam-vocabulary-game?category=${selectedCategory}`}>
                <Card className="bg-gradient-to-br from-sky-100 to-sky-300 hover:from-sky-200 hover:to-sky-400 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg h-full">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-sky-600 text-lg">
                      Listen in English, find the Malayalam word
                    </p>
                  </CardContent>
                </Card>
              </Link>
        ...
        {selectedCategoryData && (
          <div className="text-center mb-4">
            <p className="text-lg text-white font-semibold">
              Selected: {selectedCategoryData.nameMalayalam} ({selectedCategoryData.nameEnglish}) - {selectedCategoryData.items.length} words
            </p>
          </div>
        )}
        ...
        <Card className="w-full max-w-[900px] bg-white/90 backdrop-blur-sm shadow-2xl border-0">
          <CardContent className="p-6">
            <h2 className="text-2xl font-bold text-cyan-800 mb-4 text-center">
              ഏത് വിഭാഗം പഠിക്കണം? (Select Category)
            </h2>
