...
export default function LessonPage() {
  ...
  const { speakNativeAsync } = useLanguageSpeak("kannada")
  const [isPlayingTTS, setIsPlayingTTS] = useState(false)

  const handleBackToMenu = () => {
    window.location.href = "/kannada-comprehension"
  }
  ...
  if (!lesson) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 flex items-center justify-center">
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 p-4 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto mb-4">
        <Button
          ...
          className="bg-white/20 hover:bg-white/30 text-amber-900 border-2 border-white font-bold"
          variant="outline"
        >
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardContent className="p-8">
            ...
            <div className="space-y-4">
              {lesson.passage.map((line, index) => (
                <div key={index}>
                  <p className="text-2xl text-amber-900 leading-relaxed">{line}</p>
                  <p className="text-base text-amber-600/70 italic leading-relaxed">{romanize(line, "kannada")}</p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <div className="space-y-6">
          ...
          <Link href={`/kannada-comprehension/lesson/${lessonId}/game1`}>
          ...
          <Link href={`/kannada-comprehension/lesson/${lessonId}/game2`}>
            <Card className="bg-gradient-to-br from-purple-100 to-pink-300 hover:from-purple-200 hover:to-pink-400 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg border-4 border-white">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-xl text-purple-700 mb-4">
                  ಖಾಲಿ ಜಾಗ ತುಂಬಿ (Fill in the Blanks)
                </p>
