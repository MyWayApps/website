...
export default function Game2Page() {
  ...
  const { speakNativeAsync, isSpeaking: isPlayingTTS } = useLanguageSpeak("kannada")
  ...
  const handleNext = async () => {
    ...
    const applicationId = `kannada-comprehension-lesson-${lessonId}-game2`
    await saveGameScore({
      userId: user.id,
      applicationId,
      score,
    ...
    const tier = await getActivityMasteryTier(user.id, applicationId, `kannada-comprehension:${lessonId}:game2`)
    setMasteryTier(tier)
  }
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-base text-purple-700/70 italic mt-1">{romanize(getSentenceWithAnswer(), "kannada")}</p>
              </div>
              <Button
