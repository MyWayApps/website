...
import { comprehensionLessons } from "@/lib/kannada-comprehension-data"
...
export default function KannadaComprehensionMain() {
        ...
        comprehensionLessons.map(async (lesson) => {
          const [game1Tier, game2Tier] = await Promise.all([
            getActivityMasteryTier(
              user.id,
              `kannada-comprehension-lesson-${lesson.id}-game1`,
              `kannada-comprehension:${lesson.id}:game1`,
            ),
            getActivityMasteryTier(
              user.id,
              `kannada-comprehension-lesson-${lesson.id}-game2`,
              `kannada-comprehension:${lesson.id}:game2`,
            ),
          ])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 p-4 flex flex-col items-center relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        {comprehensionLessons.map((lesson, index) => (
          <Link key={lesson.id} href={`/kannada-comprehension/lesson/${lesson.id}`}>
            <Card className="bg-gradient-to-br from-white to-amber-100 hover:from-amber-50 hover:to-amber-200 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg border-4 border-white">
              <CardContent className="p-6">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="bg-amber-500 text-white rounded-full w-12 h-12 flex items-center justify-center font-bold text-xl">
                    {index + 1}
                  </div>
