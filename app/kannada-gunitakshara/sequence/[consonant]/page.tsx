...
import { gunitaksharaMatraSequence } from "@/lib/kannada-gunitakshara-data"

export default function KannadaSequenceGame() {
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:kannada-gunitakshara:sequence:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const generateQuestions = () => {
    const questions = []
    const totalMatras = gunitaksharaMatraSequence.length

    for (let i = 0; i < 5; i++) {
      const startIndex = Math.floor(Math.random() * (totalMatras - 5))
      const questionSequence = gunitaksharaMatraSequence.slice(startIndex, startIndex + 6)
      ...
      while (wrongOptions.length < 2) {
        ...
        const wrongMat = gunitaksharaMatraSequence[randomIndex]
  ...
  const handleBackToSequence = () => {
    window.location.href = "/kannada-gunitakshara/sequence"
  }
