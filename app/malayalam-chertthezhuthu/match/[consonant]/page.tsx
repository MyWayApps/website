...
import { chertthezhuthuMatraData, chertthezhuthuOptionText } from "@/lib/malayalam-chertthezhuthu-data"

export default function MalayalamMatchGame() {
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:malayalam-chertthezhuthu:match:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const shuffledMatras = [...chertthezhuthuMatraData].sort(() => Math.random() - 0.5).slice(0, numPairs)

      // Create left side options (half-letter consonant + vowel formula)
      const leftOptions = shuffledMatras.map(mat => ({
        id: `left-${mat.matra}`,
        text: chertthezhuthuOptionText(consonant, mat.matra, mat.name),
        matra: mat.matra,
        matraName: mat.name,
      ...
      const remainingMatras = chertthezhuthuMatraData.filter(mat =>
  ...
  const handleBackToMatch = () => {
    window.location.href = "/malayalam-chertthezhuthu/match"
  }
