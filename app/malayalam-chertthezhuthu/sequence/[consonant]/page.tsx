...
import { chertthezhuthuMatraSequence } from "@/lib/malayalam-chertthezhuthu-data"

export default function MalayalamSequenceGame() {
  ...
  useEffect(() => {
    getApplicationByName("Malayalam Chertthezhuthu").then((application) => setApplicationId(application?.id ?? null))
  }, [])
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:malayalam-chertthezhuthu:sequence:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  const generateQuestions = () => {
    const questions = []
    const totalMatras = chertthezhuthuMatraSequence.length

    for (let i = 0; i < 5; i++) {
      const startIndex = Math.floor(Math.random() * (totalMatras - 5))
      const questionSequence = chertthezhuthuMatraSequence.slice(startIndex, startIndex + 6)
      ...
      while (wrongOptions.length < 2) {
        ...
        const wrongMat = chertthezhuthuMatraSequence[randomIndex]
  ...
  useEffect(() => {
    if (currentQ) {
      ...
      setTimeout(() => {
        speakNativeAsync(currentQ.firstLetter).catch(err => {
  ...
  const handleBackToSequence = () => {
    window.location.href = "/malayalam-chertthezhuthu/sequence"
  }
