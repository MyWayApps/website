...
export default function MalayalamMissingGame() {
  ...
  const finishGame = async (finalScore: number) => {
    ...
    const tier = await getActivityMasteryTier(
      user.id,
      applicationId,
      `barakhadi:malayalam-chertthezhuthu:missing:${consonant}`,
    )
    setMasteryTier(tier)
  }
  ...
  useEffect(() => {
    if (currentQ) {
      ...
      setTimeout(() => {
        ...
        speakNativeAsync(fullSequence).catch(err => {
  ...
  const handleBackToMissing = () => {
    window.location.href = "/malayalam-chertthezhuthu/missing"
  }
