...
import { hindiVowels, hindiConsonants, LetterType } from "@/lib/hindi-letters-data"
import { romanize } from "@/lib/transliteration"

export default function HindiLettersFlashcardsPage() {
  ...
  const initialLetters = initialType === "consonants" ? hindiConsonants : hindiVowels
  const [selected, setSelected] = useState<string | null>(initialLetters[0]?.letter ?? null)

  const letters = tab === "vowels" ? hindiVowels : hindiConsonants
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-300 via-red-400 to-rose-500">
      <header className="bg-white/15 backdrop-blur-sm border-b-2 border-white/30 px-4 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button
            onClick={() => router.push("/hindi-letters")}
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6">
          {(["vowels", "consonants"] as const).map((t) => (
            <button
              ...
              onClick={() => {
                setTab(t)
                const newLetters = t === "vowels" ? hindiVowels : hindiConsonants
                setSelected(newLetters[0]?.letter ?? null)
              }}
              className={`flex-1 py-3 rounded-2xl font-black text-sm transition-all ${
                tab === t ? "bg-white text-red-500 shadow-lg scale-105" : "bg-white/20 text-white hover:bg-white/30"
              }`}
            >
        ...
        <p className="text-center text-white/60 text-sm mt-8 font-semibold">💡 Tap any letter to hear it spoken in Hindi</p>
      </div>
    </div>
  )
}
