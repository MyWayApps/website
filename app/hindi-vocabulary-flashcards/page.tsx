...
export default function HindiVocabularyFlashcards() {
  const searchParams = useSearchParams()
  const categoryId = searchParams.get("category") || "days"
  const { speakNative: speak, isSpeaking } = useLanguageSpeak("hindi")
  ...
  // Auto-play Hindi audio when card changes (Hindi is the front)
  useEffect(() => {
    if (items.length === 0) return
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-orange-200 to-red-500">
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-orange-200 to-red-500 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[600px] mb-6 flex items-center justify-between">
        ...
        <div className="bg-white/30 px-4 py-2 rounded-full">
          <span className="text-red-800 font-bold text-lg">{categoryName.hindi}</span>
          <span className="text-red-600 ml-2">({categoryName.english})</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 md:gap-8">
        ...
        <Button
          onClick={prev}
          variant="outline"
          className="bg-red-100 hover:bg-red-200 text-red-800 border-2 border-red-400 font-bold text-lg px-4 py-3"
        >
        ...
        >
          ...
          >
            ...
            <div className="invisible text-5xl md:text-6xl font-bold whitespace-nowrap px-8" aria-hidden="true">
              {items[index].hindi.length >= items[index].english.length ? items[index].hindi : items[index].english}
            </div>
            {/* Front - Hindi */}
            <CardContent
              className="absolute inset-0 flex flex-col items-center justify-center p-8 bg-gradient-to-br from-red-100 to-red-300 rounded-xl backface-hidden"
              style={{ backfaceVisibility: 'hidden' }}
            >
              <div className="text-sm text-red-600 mb-2 font-semibold">हिंदी</div>
              <div className="text-5xl md:text-6xl font-bold break-words text-red-800 text-center mb-1">
                {items[index].hindi}
              </div>
              <div className="text-lg md:text-xl text-red-600/80 italic text-center mb-6">
                {romanize(items[index].hindi, "devanagari")}
              </div>
              <Button
                onClick={(e) => {
                  e.stopPropagation()
                ...
                variant="outline"
                className="bg-red-200 hover:bg-red-300 text-red-800 border-2 border-red-400 px-6 py-4"
                disabled={isSpeaking}
              >
