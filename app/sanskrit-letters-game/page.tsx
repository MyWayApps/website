  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Sanskrit Letters")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Sanskrit Letters scoring data:", error)
    }
  }
    ...
    if (round > ROUND_LENGTH) {
      ...
      if (user && app) {
        saveGameScore({
          ...
          .then(() =>
            getActivityMasteryTier(user.id, app.id, `letters-game:sanskrit`)
              .then(setMasteryTier)
              .catch((error) => console.error("Error fetching Sanskrit Letters mastery tier:", error)),
          )
          .catch((error) => console.error("Error saving Sanskrit Letters score:", error))
      }
      return
    }
