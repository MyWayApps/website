...
export default function KannadaVocabularyGame() {
  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Kannada Vocabulary")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Kannada Vocabulary scoring data:", error)
    }
  }
    ...
    if (round > totalRounds) {
      ...
      if (user && app) {
        saveGameScore({
          ...
          .then(() =>
            getActivityMasteryTier(user.id, app.id, `vocabulary-game:kannada`)
              .then(setMasteryTier)
              .catch((error) => console.error("Error fetching Kannada Vocabulary mastery tier:", error)),
          )
          .catch((error) => console.error("Error saving Kannada Vocabulary score:", error))
      }
      return
    }
  ...
  if (!isReady || items.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 p-4 flex flex-col items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 p-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-[800px] flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white/30 px-4 py-2 rounded-full">
            <span className="text-amber-800 font-bold">{categoryName.kannada}</span>
          </div>
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-amber-900">
          Listen and find the Kannada word!
        </h1>
      </div>
        ...
        <CardContent className="p-8">
          {gameOver ? (
            <div className="flex flex-col items-center justify-center w-full py-8">
              <div className="text-4xl font-bold text-yellow-800 mb-4">🎉 Game Over!</div>
              ...
              <Button
                ...
                className="bg-amber-500 hover:bg-amber-600 text-white font-bold px-8 py-4 text-lg"
              >
                Play Again
              </Button>
            </div>
            ...
            <>
              <div className="mb-6 flex flex-col items-center">
                <div className="text-xl text-amber-900 font-semibold mb-4">
                  Question {round} of {totalRounds}
                </div>

                {/* English Word Display */}
                <div className="bg-gradient-to-br from-yellow-100 to-yellow-200 rounded-xl p-6 mb-4 text-center shadow-lg">
                  <div className="text-sm text-yellow-700 mb-1">Listen to the English word:</div>
                  <div className="text-3xl font-bold text-yellow-800 mb-3">
                    {items[correctIdx].english}
                  </div>
                  <Button
                    onClick={handleReplayAudio}
                    className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold text-lg px-6 py-3"
              ...
              {/* Kannada Word Choices */}
              <div className="text-center mb-4">
                <div className="text-lg text-amber-700 font-semibold">Select the correct Kannada word:</div>
              </div>
                  ...
                  <Card
                    ...
                    className={`p-6 flex items-center justify-center text-3xl md:text-4xl font-bold cursor-pointer transition-all duration-200 ${
                      showResult && choices[idx] === correctIdx && showResult === "correct"
                        ...
                        : "bg-white hover:bg-amber-50 border-2 border-amber-200 hover:border-amber-400"
                    }`}
