...
import { getLettersByType, getLetterTypeLabel, LetterType, HindiLetter } from "@/lib/hindi-letters-data"
...
export default function HindiLettersGame() {
  ...
  const [letters, setLetters] = useState<HindiLetter[]>([])
  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Hindi Letters")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Hindi Letters scoring data:", error)
    }
  }
    ...
    if (round > ROUND_LENGTH) {
      ...
      if (user && app) {
        saveGameScore({
          ...
          .then(() =>
            getActivityMasteryTier(user.id, app.id, `letters-game:hindi`)
              .then(setMasteryTier)
              .catch((error) => console.error("Error fetching Hindi Letters mastery tier:", error)),
          )
          .catch((error) => console.error("Error saving Hindi Letters score:", error))
      }
      return
    }
    const correct = pickUnseenRandom(`hindi-letters-game:${letterType}`, 0, letters.length - 1)
  ...
  const handleBackToHome = () => {
    window.location.href = "/hindi-letters"
  }
  const handleRestart = () => {
    setScore(0)
    setRound(1)
  ...
  if (!isReady || letters.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-300 via-red-400 to-rose-500 p-4 flex flex-col items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-300 via-red-400 to-rose-500 p-4 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <CardContent className="p-8">
          {gameOver ? (
            <div className="flex flex-col items-center justify-center w-full">
              ...
              <div className="text-2xl text-red-900 mb-2">Your Score: {score} / {ROUND_LENGTH}</div>
              {masteryTier && (
                <div className="mb-4">
                  <MasteryBadge tier={masteryTier} showLabel />
                </div>
              )}
              <Button onClick={handleRestart} className="mt-4 bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-3" variant="outline">
                Play Again
              </Button>
            </div>
          ) : (
            <>
              <div className="mb-6 flex flex-col items-center">
                <div className="text-xl text-red-900 font-semibold mb-3">
                  Question {round} of {ROUND_LENGTH}
                </div>
                <Button
                  onClick={handleReplayAudio}
                  disabled={isSpeaking}
                  className="bg-white/30 hover:bg-white/40 text-red-800 border-white font-bold text-lg px-6 py-3"
                  variant="outline"
                >
                  🔊 Hear the Letter
                </Button>
                ...
                {choices.map((letterIdx, idx) => (
                  ...
                  >
                    ...
                    <span className="text-base font-normal opacity-70">
                      {romanize(letters[letterIdx].letter, "devanagari")}
                    </span>
                  </Card>
                ))}
              </div>
