...
export default function Game1Page() {
  ...
  const handleNext = async () => {
    ...
    const applicationId = `telugu-comprehension-lesson-${lessonId}-game1`
    await saveGameScore({
      userId: user.id,
      applicationId,
      score,
    ...
    const tier = await getActivityMasteryTier(user.id, applicationId, `telugu-comprehension:${lessonId}:game1`)
    setMasteryTier(tier)
  }
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-base text-green-700/70 italic mt-1">{romanize(currentQuestion.question, "telugu")}</p>
              </div>
              <Button
