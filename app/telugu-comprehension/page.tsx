...
import { comprehensionLessons } from "@/lib/telugu-comprehension-data"
...
export default function TeluguComprehensionMain() {
        ...
        comprehensionLessons.map(async (lesson) => {
          const [game1Tier, game2Tier] = await Promise.all([
            getActivityMasteryTier(
              user.id,
              `telugu-comprehension-lesson-${lesson.id}-game1`,
              `telugu-comprehension:${lesson.id}:game1`,
            ),
            getActivityMasteryTier(
              user.id,
              `telugu-comprehension-lesson-${lesson.id}-game2`,
              `telugu-comprehension:${lesson.id}:game2`,
            ),
          ])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-200 to-blue-400 p-4 flex flex-col items-center relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        {comprehensionLessons.map((lesson, index) => (
          <Link key={lesson.id} href={`/telugu-comprehension/lesson/${lesson.id}`}>
            <Card className="bg-gradient-to-br from-white to-blue-100 hover:from-blue-50 hover:to-blue-200 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg border-4 border-white">
              <CardContent className="p-6">
                ...
                <div className="mt-4 pt-4 border-t border-blue-300">
                  <div className="flex justify-around text-sm text-blue-700">
                    <div className="text-center">
