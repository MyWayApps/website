...
import { getCategoryById, VocabularyItem } from "@/lib/hindi-vocabulary-data"
...
export default function HindiVocabularyGame() {
  const searchParams = useSearchParams()
  const categoryId = searchParams.get("category") || "days"

  const [items, setItems] = useState<VocabularyItem[]>([])
  const [categoryName, setCategoryName] = useState({ hindi: "", english: "" })
  ...
  const initializeScoringData = async () => {
    try {
      ...
      if (connected) {
        ...
        const application = await getApplicationByName("Hindi Vocabulary")
        setUser(currentUser)
        setApp(application)
      } else {
        const userData = localStorage.getItem("mywayapps_current_user")
      ...
      console.error("Error initializing Hindi Vocabulary scoring data:", error)
    }
  }
  ...
  useEffect(() => {
    ...
    if (category) {
      ...
      setCategoryName({ hindi: category.nameHindi, english: category.nameEnglish })
    ...
    if (round > totalRounds) {
      ...
      if (user && app) {
        saveGameScore({
          ...
          .then(() =>
            getActivityMasteryTier(user.id, app.id, `vocabulary-game:hindi`)
              .then(setMasteryTier)
              .catch((error) => console.error("Error fetching Hindi Vocabulary mastery tier:", error)),
          )
          .catch((error) => console.error("Error saving Hindi Vocabulary score:", error))
      }
      return
    }

    const correct = pickUnseenRandom(`hindi-vocabulary-game:${categoryId}`, 0, items.length - 1)
  ...
  const handleBackToVocabulary = () => {
    window.location.href = "/hindi-vocabulary"
  }
  ...
  if (!isReady || items.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-200 to-red-500 p-4 flex flex-col items-center justify-center">
        <div className="text-2xl text-red-800">Loading...</div>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-200 to-red-500 p-4 flex flex-col items-center justify-center relative overflow-hidden">
      {/* Decorative Character */}
      <img
        src="/characters/parrot.png"
        alt="Parrot"
      ...
      <div className="w-full max-w-[800px] flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white/30 px-4 py-2 rounded-full">
            <span className="text-red-800 font-bold">{categoryName.hindi}</span>
          </div>
          <div className="flex items-center gap-2 bg-white/20 px-6 py-3 rounded-full backdrop-blur-sm">
            <Star className="h-6 w-6 text-yellow-500" />
            <span className="text-xl font-bold text-red-800">Score: {score}</span>
          </div>
        </div>
      </div>
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-red-900">
          Listen and find the Hindi word!
        </h1>
      </div>
        ...
        <CardContent className="p-8">
          {gameOver ? (
            <div className="flex flex-col items-center justify-center w-full py-8">
              ...
              <div className="text-2xl text-red-900 mb-2">Your Score: {score} / {totalRounds}</div>
              {masteryTier && (
                <div className="mb-4">
            ...
            <>
              <div className="mb-6 flex flex-col items-center">
                <div className="text-xl text-red-900 font-semibold mb-4">
                  Question {round} of {totalRounds}
                </div>

                {/* English Word Display */}
                <div className="bg-gradient-to-br from-orange-100 to-orange-200 rounded-xl p-6 mb-4 text-center shadow-lg">
                  <div className="text-sm text-orange-600 mb-1">Listen to the English word:</div>
                  <div className="text-3xl font-bold text-orange-800 mb-3">
              ...
              <div className="text-center mb-4">
                <div className="text-lg text-red-700 font-semibold">Select the correct Hindi word:</div>
              </div>
                ...
                {choices.map((itemIdx, idx) => (
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <span className="text-red-800">{items[itemIdx].hindi}</span>
                      <span className="text-base md:text-lg font-normal text-red-600/80 italic mt-1">
                        {romanize(items[itemIdx].hindi, "devanagari")}
                      </span>
                    </div>
                  </Card>
                ))}
