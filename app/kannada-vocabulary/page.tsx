...
import { vocabularyCategories } from "@/lib/kannada-vocabulary-data"

export default function KannadaVocabulary() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 to-amber-500 p-4 relative overflow-hidden">
        ...
        <Card className="w-full max-w-[900px] bg-white/90 backdrop-blur-sm shadow-2xl border-0 mb-6">
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Link href={`/kannada-vocabulary-flashcards?category=${selectedCategory}`}>
                <Card className="bg-gradient-to-br from-yellow-100 to-yellow-300 hover:from-yellow-200 hover:to-yellow-400 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg h-full">
              ...
              {/* Vocabulary Game */}
              <Link href={`/kannada-vocabulary-game?category=${selectedCategory}`}>
                <Card className="bg-gradient-to-br from-amber-100 to-amber-300 hover:from-amber-200 hover:to-amber-400 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg h-full">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-amber-600 text-lg">
                      Listen in English, find the Kannada word
                    </p>
                  </CardContent>
                </Card>
              </Link>
        ...
        {selectedCategoryData && (
          <div className="text-center mb-4">
            <p className="text-lg text-white font-semibold">
              Selected: {selectedCategoryData.nameKannada} ({selectedCategoryData.nameEnglish}) - {selectedCategoryData.items.length} words
            </p>
          </div>
        )}
        ...
        <Card className="w-full max-w-[900px] bg-white/90 backdrop-blur-sm shadow-2xl border-0">
          <CardContent className="p-6">
            <h2 className="text-2xl font-bold text-amber-800 mb-4 text-center">
              ಯಾವ ವಿಭಾಗ ಕಲಿಯಬೇಕು? (Select Category)
            </h2>
