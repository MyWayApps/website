...
export default function Game1Page() {
  ...
  const { speakNativeAsync, isSpeaking: isPlayingTTS } = useLanguageSpeak("malayalam")
  ...
  const handleBackToLesson = () => {
    window.location.href = `/malayalam-comprehension/lesson/${lessonId}`
  }
  ...
  const handleNext = async () => {
    ...
    const applicationId = `malayalam-comprehension-lesson-${lessonId}-game1`
    await saveGameScore({
      userId: user.id,
      applicationId,
      score,
    ...
    const tier = await getActivityMasteryTier(user.id, applicationId, `malayalam-comprehension:${lessonId}:game1`)
    setMasteryTier(tier)
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-200 to-emerald-400 p-4 relative overflow-hidden">
      ...
      <div className="text-center mb-6">
        <h1 className="text-4xl font-bold text-green-900 mb-2">
          {lesson.malayalamTitle} - Question Set 1
        </h1>
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-base text-green-700/70 italic mt-1">{romanize(currentQuestion.question, "malayalam")}</p>
              </div>
              <Button
                ...
                <CardContent className="p-6 flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 italic">{romanize(option, "malayalam")}</p>
                    </div>
                  </div>
