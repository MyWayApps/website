...
import { comprehensionLessons } from "@/lib/malayalam-comprehension-data"
...
export default function MalayalamComprehensionMain() {
        ...
        comprehensionLessons.map(async (lesson) => {
          const [game1Tier, game2Tier] = await Promise.all([
            getActivityMasteryTier(
              user.id,
              `malayalam-comprehension-lesson-${lesson.id}-game1`,
              `malayalam-comprehension:${lesson.id}:game1`,
            ),
            getActivityMasteryTier(
              user.id,
              `malayalam-comprehension-lesson-${lesson.id}-game2`,
              `malayalam-comprehension:${lesson.id}:game2`,
            ),
          ])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-300 via-cyan-400 to-sky-500 p-4 flex flex-col items-center relative overflow-hidden">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-2xl text-cyan-800 font-semibold">
          Malayalam Comprehension
        </p>
        <p className="text-xl text-cyan-700 mt-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        {comprehensionLessons.map((lesson, index) => (
          <Link key={lesson.id} href={`/malayalam-comprehension/lesson/${lesson.id}`}>
            <Card className="bg-gradient-to-br from-white to-cyan-100 hover:from-cyan-50 hover:to-cyan-200 cursor-pointer transition-all duration-300 hover:scale-105 shadow-lg border-4 border-white">
              <CardContent className="p-6">
                {/* Lesson Number Badge */}
                <div className="flex justify-between items-start mb-4">
                  <div className="bg-cyan-500 text-white rounded-full w-12 h-12 flex items-center justify-center font-bold text-xl">
                    {index + 1}
                  </div>
                ...
                <div className="text-center">
                  <h3 className="text-3xl font-bold text-cyan-900 mb-2">
                ...
                <div className="mt-4 pt-4 border-t border-cyan-300">
                  <div className="flex justify-around text-sm text-cyan-700">
                    <div className="text-center">
